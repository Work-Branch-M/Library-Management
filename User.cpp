...
void User::returnBook() {
    ...
    // Build a filtered list only for this user and show a formatted table
    std::vector<size_t> userIdx; userIdx.reserve(rows.size());
    ...
    for (size_t i=0;i<rows.size();++i) {
        ...
        if (p[0]==name && p[1]==studentId) {
            userIdx.push_back(i);
            std::cout << std::left
                      << std::setw(4)  << userIdx.size()
                      << std::setw(8)  << p[2]
                      << std::setw(30) << p[3].substr(0,29)
                      << std::setw(20) << p[4].substr(0,19) << "\n";
        }
    }
    if (userIdx.empty()) { std::cout << "You have no borrowed books." << std::endl; return; }
    ...
    if (idx<1 || idx>(int)userIdx.size()) { std::cout << "Out of range." << std::endl; return; }

    // Map selection to the corresponding global row index
    auto p = rows[userIdx[idx-1]];
    ...
    for (size_t i=0;i<rows.size();++i) {
        if (i==userIdx[idx-1]) continue; // skip returned
        auto &r = rows[i];
        if (r.size()>=5)
        ...
        try {
            ...
            if (res) {
                ...
                std::cout << std::setw(8)  << res->id
                          << std::setw(30) << res->title.substr(0,29)
                          << std::setw(20) << res->category.substr(0,19) << "\n";
            }
